...
if instance_exists(oInvisibleSpawner2){//If spawner exists...
	if global.enemyKillCount == oInvisibleSpawner2.maxTotalEnemies && playerHp != 0{//If the  player reaches the total number of enemies to kill.
	    ...
	    instance_deactivate_all(oHUD);//Deactivates HUD.

	    if !instance_exists(oVictoryScreen){//If victory screen does not already exist...
	        instance_create_depth(0, 0, 0, oVictoryScreen);//Create a victory screen.
	    }
